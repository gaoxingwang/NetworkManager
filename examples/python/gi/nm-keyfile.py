...
def kf_handler_read(keyfile,
    ...
    if handler_type == NM.KeyfileHandlerType.WRITE_CERT:
        ...
        # for read.
        handler_data.fail_with_error(GLib.GError.new_literal(NM.ConnectionError.quark(), 'hallo1', NM.ConnectionError.MISSINGPROPERTY))
...
print("parsing succeeded: \"%s\" (%s)%s" % (c.get_id(), c.get_uuid(), " (invalid: " + verify_failure + ")" if verify_failure is not None else ""))


###############################################################################

def kf_handler_write(connection,
                     keyfile,
                     handler_type,
                     handler_data,
                     user_data):
    ...
    debug("kf_handler_write(%s): handler-type=%r" % (kf_handler_write_cnt, handler_type))
    debug("kf_handler_write(%s): handler-data=%r" % (kf_handler_write_cnt, handler_data))
